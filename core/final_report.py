...
def build_final_report(verdict: dict[str, Any]) -> dict[str, Any]:
    ...
    reasons = _unique_texts(verdict.get("reasons") or [], limit=5)
    steps = _unique_texts(verdict.get("next_steps") or [], limit=5)
    ...
    closeout = (verdict.get("cross_temporal_analysis") or {}).get("closeout_report") or {}
    agreements = _unique_texts(judge.get("agreements") or deliberation.get("agreements") or [], limit=6)
    disagreements = _unique_texts(judge.get("disagreements") or deliberation.get("disagreements") or [], limit=4)
    limits = _unique_texts(judge.get("limits") or [], limit=5)
    ...
    plan = _implementation_plan(steps, complete=complete, coverage=coverage, verdict_label=verdict_label)
    ...
    return {
        ...
        "title": "AI Judge 最终方案报告",
        "subtitle": "FINAL VERDICT · EXECUTIVE PLAN · AUDITABLE SYNTHESIS",
        ...
        "final_position": {
            ...
            "confidence": confidence,
            "trust": trust,
            "summary": _final_position_summary(verdict_label, trust, coverage, reasons),
        },
...
def render_final_report_html(report: dict[str, Any]) -> str:
    ...
    return (
        ...
        f'<p class="paper-status">{html.escape(str(report.get("status_label", "-")))} · {html.escape(str(report.get("status_reason", "-")))}</p>'
        "</div>"
...
def _abstract(
    *,
    ...
    closeout_summary = _text(closeout.get("professional_report") or closeout.get("executive_summary"))
    if closeout_summary:
        lead = _compact(closeout_summary, 280)
    else:
        lead = f"本报告围绕“{_compact(question, 90)}”给出 AI Judge 的最终方案：{verdict_label}。"
    reason_text = "；".join(reasons[:3]) or _text(verdict.get("one_liner") or "当前证据仍需继续补强")
    step_text = "；".join(steps[:2]) or "先补齐证据链，再进入执行或发布确认"
    boundary = "可作为最终方案进入执行复核。" if complete else "因席位未全量闭环，只能作为阶段性方案，不能包装为全模型最终共识。"
    return (
        f"{lead} 判定可信度为 {confidence}，席位覆盖 {coverage['label']}。"
        f"核心依据是：{reason_text}。建议执行路径是：{step_text}。{boundary}"
    )
...
def _postulates(
    *,
    ...
    reason_text = "；".join(reasons[:3]) or "暂无足够稳定的共同理由"
    agreement_text = "；".join(agreements[:4]) or "席位共识尚未形成高密度表达"
    top_text = "、".join(top_seats[:4]) or "已返回席位"
    return [
        {
            "title": "最终立场先于细节展开",
            "body": f"法官结论为“{verdict_label}”，置信度 {confidence}；后续所有建议都必须服务于这个主结论，而不是堆叠模型原文。",
            "evidence": reason_text,
        },
        {
            "title": "共识只采纳可追溯来源",
            "body": f"本报告优先采用 {top_text} 的共同部分，并把席位覆盖 {coverage['label']} 作为可信边界。",
            "evidence": agreement_text,
        },
        {
            "title": "最终方案必须可执行",
...
def _final_position_summary(
    ...
    reason_text = "；".join(reasons[:3]) or "当前理由不足，需要继续补证"
    return f"最终立场为“{verdict_label}”，可信等级 {trust or '-'}，席位覆盖 {coverage['label']}；主要依据是：{reason_text}。"
