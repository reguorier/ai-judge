#!/usr/bin/env python3
"""Generate Market Simulation Layer acceptance artifacts."""
...
ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / "20260617-market-simulation-acceptance-v2"
...
def main() -> None:
    ...
    bundles = [
        write_report_bundle(_report("msl-accept-run-0", "noise"), reports_root=REPORTS_ROOT),
        write_report_bundle(_report("msl-accept-run-1", "price"), reports_root=REPORTS_ROOT),
        write_report_bundle(_report("msl-accept-run-2", "price"), reports_root=REPORTS_ROOT),
        write_report_bundle(_report("msl-accept-run-3", "liquidity"), reports_root=REPORTS_ROOT),
        write_report_bundle(_report("msl-accept-run-4", "reversal"), reports_root=REPORTS_ROOT),
    ]
    latest = bundles[-1]
    manifest = {
        "schema": "ai_judge.market_simulation_acceptance.v1",
        ...
        "primary_review_files": {
            ...
            "market_simulation_state": latest["paths"]["market_simulation_state"],
    ...
    shutil.copy2(latest["paths"]["market_simulation_state"], ARTIFACT_ROOT / "market_simulation_state.json")
...
def _report(run_id: str, pattern: str) -> dict:
    if pattern == "price":
        question = "Product MSL: 世界杯预测池中盘口、赔率和模型信号分歧时，策略在不同市场世界下是否稳健？"
        seats = [
        ...
        question = "Product MSL: 当热门球队吸引公众资金导致赔率快速移动，策略是否会被流动性冲击放大风险？"
        seats = [
        ...
        question = "Product MSL: 若临场伤病或阵容消息反转原始信号，生产策略在 favorite reversal 世界下表现如何？"
        seats = [
        ...
        question = "Product MSL: 当模型输出污染、旧上下文残留和过度自信并存时，模拟层应如何暴露策略脆弱性？"
        seats = [
    ...
    return build_client_final_report(
        ...
        failed_seats=0,
        generated_at=f"2026-06-17T04:0{run_id[-1]}:00+08:00",
        seat_outputs=[
            {
                "seat_id": seat_id,
...
def _run_manifest(bundle: dict) -> dict:
    ...
    return {
        ...
        "scenario_count": summary.get("msl_scenario_count"),
        "monte_carlo_runs": summary.get("msl_monte_carlo_runs"),
        "robust_strategy_count": summary.get("msl_robust_strategy_count"),
        "top_scenario_risk": summary.get("msl_top_scenario_risk"),
        "worst_scenario_id": summary.get("msl_worst_scenario_id"),
        "runtime_view": bundle["paths"]["runtime_view"],
        "market_simulation": bundle["paths"]["market_simulation"],
        "market_simulation_state": bundle["paths"]["market_simulation_state"],
    }
