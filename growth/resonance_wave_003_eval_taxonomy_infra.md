...
|---|---:|---|---|---|---|
| W003-001 | P0 | Eval4SD 2026 | Workshop submission | `papers/eval4sd2026/openreview_submission.md` | openreview_login_blocked_packet_verified |
...
Continue by recovering OpenReview login or requesting venue/account support before opening the Eval4SD submission form. The submission page and "Add: GSCL KONVENS 2026 Workshop Eval4SD Submission" route are live, but OpenReview currently returns "Please Login to proceed." The OpenReview signup confirmation email arrived, the activation link was opened, and the profile registration form was completed with a public project URL, independent-researcher history, and relevant expertise tags. Two saved-login attempts on 2026-05-18 returned to the login page without establishing a session, and the "Didn't receive email confirmation?" link did not expose a resend flow in the UI. Do not inspect saved passwords or perform a password reset through automation; use user-assisted login, OpenReview support, or organizer guidance.

Once login is recovered, upload `papers/eval4sd2026/main.pdf` and use `papers/eval4sd2026/openreview_submission.md` for form fields. Stop before the final submit button until action-time confirmation is available. The Eval4SD packet was rechecked on 2026-05-18: the PDF remains anonymous and the full/hard benchmarks both pass at 100%. Eval4SD fit is confirmed by organizer reply: short/position paper is a good fit, and a demo paper with limited evaluation would also work; no further pre-fit check is needed. Follow up on sent W003-002 and W003-005 only after 3-5 days or if a reply arrives. W003-003 and RAGChecker were checked by `gh issue view` on 2026-05-18 and have no comments yet, so do not post a no-op follow-up. For W003-004, do not guess an unverified email address; retry from a clean DNS/network path or a verified public channel before sending.
...
- W003-001 OpenReview: the submission route is live and points to OpenReview. The confirmation email arrived, the activation link was opened, and profile registration was completed. The account currently cannot establish a logged-in session through saved Chrome credentials, so submission is blocked at login rather than at packet readiness. Use the pushed packet and PDF after login. Final submit still needs action-time confirmation.
