...
The current claim-support block covers a narrow but important failure family: a real and relevant source that supports a weaker proposition than the model states. The deterministic MVP catches association upgraded into causation, absolute claims contradicted by limited evidence, and inflated percentage claims. In those cases, the system can report verified citation status, relevant source status, contradicted claim support, and a concrete failure code such as \texttt{overclaimed\_causation}, \texttt{overclaimed\_absolute}, or \texttt{overclaimed\_quantified\_effect}.

\section{Evaluation}

We use two deterministic benchmarks. The default benchmark contains 100 cases balanced across the five citation labels. The hard benchmark contains 13 public launch cases designed to exercise boundary conditions, including three cases where citation status is verified while claim support is contradicted.
...
\midrule
\texttt{bench-100} & 100 & 100 & 0 & 1.00 \\
\texttt{hard-11} & 13 & 13 & 0 & 1.00 \\
\bottomrule
\end{tabularx}
...
The hard benchmark covers fabricated sources, real-but-irrelevant sources, contradicted sources, weak matches, missing or blocked evidence, and real-source overclaimed-support cases.
...
The most important rows are not fabricated citations. They are real-source / unsupported-conclusion cases:
...
Field & Value \\
\midrule
Citation status & \texttt{verified} \\
Source relevance & \texttt{relevant} \\
Claim support & \texttt{contradicted} \\
Failure codes & \texttt{overclaimed\_causation}, \texttt{overclaimed\_absolute}, \texttt{overclaimed\_quantified\_effect} \\
\bottomrule
\end{tabularx}
...
These rows are the core methodological point: a source can be real and relevant while the generated conclusion is not supported.
